...
2. **Broker como servicio (LaunchAgent):** `~/Library/LaunchAgents/com.tuorg.claude-peers.plist`
   → broker en `:7899`, `RunAtLoad` + `KeepAlive` (arranca al login, se reinicia si muere).
   - Cargar: `launchctl load ~/Library/LaunchAgents/com.tuorg.claude-peers.plist`
   - Logs: `/tmp/claude-peers-broker.{out,err}.log`
3. **MCP en `~/.claude.json`** (scope user → vale en cualquier carpeta):
   ```json
   "claude-peers": { "type":"stdio",
     "command":"/usr/local/bin/peers-client", "args":[], "env":{} }
   ```
...
## Receta B — Servidor Linux (tu-servidor u otro): UN comando con `install.sh`
...
```bash
scp -r dist/* tu-servidor:/tmp/cprs/
ssh tu-servidor 'cd /tmp/cprs && ./install.sh'
```
...
- `CLAUDE_PEERS_REDIS_URL=redis://TU-REDIS:6379 ./install.sh` — Redis remoto.
...
## Ejemplo de setup en LAN — broker en una máquina, servidores se conectan

**Broker central: una máquina de la LAN (macOS/Linux)**, expuesta en la LAN con token.
- Broker: servicio (LaunchAgent/systemd) con `--host 0.0.0.0 --puerto 7899` + `CLAUDE_PEERS_TOKEN=<TU-TOKEN>`.
- IP del broker en la LAN: **`<IP-DEL-BROKER>`** (cambia si cambia la red — re-verificar con
  `ifconfig | grep "inet "` o `ip addr`).
- Client local (`~/.claude.json`): `env.CLAUDE_PEERS_TOKEN=<TU-TOKEN>` (apunta a
  `127.0.0.1:7899` por defecto, local).

**En cada servidor de la MISMA LAN** (vía el plugin o install.sh), el client apunta al broker.
...
mostrará los peers de la red; los mensajes cruzan en ambos sentidos.
...
> y, si hay un broker local en el server, se registra en **esa isla** (no en la máquina del broker). Por eso
> un-peer "desaparecía" de la TUI.
>
...
> en la máquina del broker). Si quedó uno corriendo de una sesión vieja: `pkill -f peers-broker-linux`
> antes de relanzar `claude` con las variables ya exportadas.

> Requiere que el server alcance `<IP-DEL-BROKER>:7899` (misma LAN, sin firewall bloqueando). Probar
> desde el server: `curl -s -o /dev/null -w "%{http_code}\n" http://<IP-DEL-BROKER>:7899/salud` → 200.

## Por qué esto supera al TS

- El TS necesitaba `bun` instalado + `git clone` + `bun install` (falló en pruebas).
